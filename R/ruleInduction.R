...
#'
#' Induces association [rules] that can be generated from supplied [itemsets],
#' optionally using a [transactions] data set to recount support.
#'
...
#' @param transactions the [transactions] used to mine `x`. This can be omitted
#' for `method = "ptree"` only when `x` is a complete collection of frequent
#' itemsets with support values.
#' @param confidence numeric value in `[0, 1]` giving the minimum confidence
#' threshold.
#' @param method induction method: `"ptree"` or `"apriori"`.
#' @param reduce logical; remove unused items before counting to reduce memory
#' use and potentially improve speed?
#' @param verbose logical; report progress and timing information?
#' @param ... unused; unknown arguments produce a warning.
#' @return A [rules] object containing all induced rules meeting the confidence
