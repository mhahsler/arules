...
discretize <- function(
  ...
  breaks <- switch(method,
    ...
    cluster = {
      ...
      as.numeric(c(min(x, na.rm = TRUE), head(
        centers,
        length(centers) - 1
      ) + diff(centers) / 2, max(x, na.rm = TRUE)))
    },
    fixed = breaks
  )
...
.rediscretizeDF <- function(data, newdata) {
  ...
  cps <- lapply(data, FUN = function(x) {
    ...
    if (is.null(breaks)) {
      ...
      list(breaks = breaks, method = "fixed", labels = levels(x))
    }
  })
