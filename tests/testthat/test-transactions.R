test_that("transaction construction and coercion", {
data <- list(
  c("a", "b", "c"),
  c("a", "b"),
  c("a", "b", "d"),
...
expect_warning(matrix_result <- as(c(trans[, -2], trans[, -3]), "matrix"))
expect_true(all(matrix_result[1:8, "b"]) == FALSE)
expect_true(all(matrix_result[9:15, "c"]) == FALSE)
