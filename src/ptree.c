...
/*
 ...
 Rewritten to consistently PROTECT slot lookups and allocated
 objects that are kept across further allocations.
 */
...
SEXP R_pncount(SEXP R_x, SEXP R_t, SEXP R_s, SEXP R_o, SEXP R_v) {
  ...
  PROTECT(dimx = GET_SLOT(R_x, install("Dim")));
  nprotect++;
  PROTECT(dimt = GET_SLOT(R_t, install("Dim")));
  nprotect++;
  if (INTEGER(dimx)[0] != INTEGER(dimt)[0])
    error("the number of rows of 'x' and 't' do not conform");

  nr = INTEGER(dimx)[0];

  PROTECT(px = GET_SLOT(R_x, install("p")));
  nprotect++;
  PROTECT(ix = GET_SLOT(R_x, install("i")));
  nprotect++;
  PROTECT(pt = GET_SLOT(R_t, install("p")));
  nprotect++;
  PROTECT(it = GET_SLOT(R_t, install("i")));
  nprotect++;

  PROTECT(r = allocVector(INTSXP, nr + 1));
  nprotect++;
  pb = INTEGER(r);
  ...
  if (LOGICAL(R_o)[0] == TRUE) {
    ...
    PROTECT(ord = allocVector(INTSXP, nr));
    nprotect++;
    o = INTEGER(ord);
  ...
  if (LOGICAL(R_s)[0] == TRUE) {
    ...
    PROTECT(oobj = NEW_OBJECT_OF_CLASS("ngCMatrix"));
    nprotect++;
    SET_VECTOR_ELT(r, 0, oobj);
    PROTECT(pl = allocVector(INTSXP, np + 1));
    nprotect++;
    SET_SLOT(oobj, install("p"), pl);
    PROTECT(il = allocVector(INTSXP, ni));
    nprotect++;
    SET_SLOT(oobj, install("i"), il);
    PROTECT(dim = allocVector(INTSXP, 2));
    nprotect++;
    INTEGER(dim)[0] = nr;
    INTEGER(dim)[1] = np;
    SET_SLOT(oobj, install("Dim"), dim);

    PROTECT(oobj = NEW_OBJECT_OF_CLASS("ngCMatrix"));
    nprotect++;
    SET_VECTOR_ELT(r, 1, oobj);
    PROTECT(pr = allocVector(INTSXP, np + 1));
    nprotect++;
    SET_SLOT(oobj, install("p"), pr);
    PROTECT(ir = allocVector(INTSXP, np));
    nprotect++;
    SET_SLOT(oobj, install("i"), ir);
    PROTECT(dim = allocVector(INTSXP, 2));
    nprotect++;
    INTEGER(dim)[0] = nr;
    INTEGER(dim)[1] = np;
    ...
    PROTECT(rs = allocVector(REALSXP, np));
    nprotect++;
    SET_VECTOR_ELT(r, 2, rs);
    PROTECT(rc = allocVector(REALSXP, np));
    nprotect++;
    SET_VECTOR_ELT(r, 3, rc);
    PROTECT(rl = allocVector(REALSXP, np));
    nprotect++;
    SET_VECTOR_ELT(r, 4, rl);
    PROTECT(pi = allocVector(INTSXP, np));
    nprotect++;
    SET_VECTOR_ELT(r, 5, pi);
  ...
  if (LOGICAL(R_o)[0] == TRUE && LOGICAL(R_s)[0] == FALSE) {
#ifdef _TIME_H
    if (LOGICAL(R_v)[0] == TRUE)
      Rprintf("recoding ... ");
#endif
...
SEXP R_pnindex(SEXP R_x, SEXP R_y, SEXP R_v) {
  int i, k, f, l, n, nr, e, nprotect = 0;
  int *x;
  SEXP r, dimx, dimy, px, ix, py, iy;
#ifdef _TIME_H
  clock_t t2, t1 = clock();
#endif

  if (!inherits(R_x, "ngCMatrix") && !inherits(R_x, "sgCMatrix"))
    error("'x' not of class ngCMatrix");
  if (!isNull(R_y) && !inherits(R_y, "ngCMatrix") && !inherits(R_y, "sgCMatrix"))
  ...
  if (!isNull(R_y)) {
    PROTECT(dimy = GET_SLOT(R_y, install("Dim")));
    nprotect++;
    if (nr != INTEGER(dimy)[0])
      error("'x' and 'y' not the same number of rows");
    PROTECT(py = GET_SLOT(R_y, install("p")));
    nprotect++;
    PROTECT(iy = GET_SLOT(R_y, install("i")));
    nprotect++;
  }
...
SEXP R_pnrindex(SEXP R_x, SEXP R_v) {
  int i, j, k, f, l, m, n, nr, nprotect = 0;
  int *x;
  SEXP dimx, px, ix;
  SEXP r, is, ir, il, pbuf;
#ifdef _TIME_H
  clock_t t2, t1 = clock();
#endif
  ...
  nprotect++;
  PROTECT(is = allocVector(INTSXP, m));
  nprotect++;
  SET_VECTOR_ELT(r, 0, is);
  PROTECT(il = allocVector(INTSXP, m));
  nprotect++;
  SET_VECTOR_ELT(r, 1, il);
  PROTECT(ir = allocVector(INTSXP, m));
  nprotect++;
  SET_VECTOR_ELT(r, 2, ir);

  PROTECT(pbuf = allocVector(INTSXP, k + 1));
  nprotect++;
  pb = INTEGER(pbuf);
