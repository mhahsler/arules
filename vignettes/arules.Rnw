...
This paper presents \pkg{arules}\footnote{The \pkg{arules} package can be obtained from \url{http://CRAN.R-Project.org}. Issues can be submitted on the package's GitHub page at \url{https://github.com/mhahsler/arules}.}, an extension package for
